...
", color:"#34D399" },
  ...
  { id:"paid",      label:"Payment Received", icon:"
...
function Toast({ message, type="success", onClose }) {
  ...
  return <div className="toast">{ {success:"
...
    unit.repairNote&&["Repair Note",<span style={{color:"#FECACA"}}>{unit.repairNote}</span>],
    ...
    unit.paymentReceived!==undefined&&unit.status==="sold"&&["Payment",unit.paymentReceived?<span style={{color:"var(--gr)"}}>

 Received</span>:<span style={{color:"var(--am)"}}>
    ...
    <div className="ov" onClick={e=>e.target===e.currentTarget&&onClose()}>
      ...
      <div className="mo lg">
        ...
        <div style={{display:"flex",alignItems:"center",gap:11,marginBottom:4}}>
          ...
          <div style={{width:40,height:40,borderRadius:9,background:"linear-gradient(135deg,rgba(56,189,248,.15),rgba(129,140,248,.15))",display:"flex",alignItems:"center",justifyContent:"center",fontSize:18,flexShrink:0}}>
...
function Login({ users, onLogin }) {
  ...
  return <div className="lw"><div className="lcard">
    ...
    <div className="lbrand"><div className="lico">

</div><div><div className="laname">Cool<em>Stock</em></div><div className="latag">AC Inventory Management v6</div></div></div>
  ...
  const TonBreakdown = ({ unitSet }) => {
    ...
    return <>{data.map((t,i)=><div key={t.label} style={{marginBottom:11}}>
      ...
      <div style={{display:"flex",justifyContent:"space-between",marginBottom:4}}>
        ...
        <div style={{display:"flex",gap:11,fontSize:10.5}}><span style={{color:"var(--am)"}}>

{t.pend}</span><span style={{color:"var(--gr)"}}>

{t.avail}</span><span style={{color:"var(--rd)"}}>

{t.rep}</span><span style={{color:"var(--gy)"}}>

{t.sold}</span><span style={{color:"var(--ac)",fontWeight:700}}>Σ{t.total}</span></div>
 ...
 {wh?.name||"—"}</span></td>
  ...
  return <div>
    ...
    <div className="ph">
      ...
      <div><div className="pt">

 Dashboard</div><div className="ps">{new Date().toLocaleDateString("en-IN",{weekday:"long",day:"numeric",month:"long",year:"numeric"})}</div></div>

    </div>

    {overall.avail<20&&<div className="al al-w">

 <div><strong>Low Stock:</strong> Only {overall.avail} units available.</div></div>}

    {pendPay>0&&<div className="al al-b">

 <div><strong>{pendPay} pending payment{pendPay>1?"s":""}</strong> — follow up with customers.</div></div>}
...
","Received",overall.total,"var(--ac)"],["

","Pending QC",overall.pend,"var(--am)"],["

","Repair",overall.rep,"var(--rd)"],["

","Available",overall.avail,"var(--gr)"],["

","Sold",overall.sold,"var(--gy)"]].map(([ic,lb,vl,cl],i)=><div className="ps2" key={i}><div style={{fontSize:15,marginBottom:2}}>{ic}</div><div className="pv" style={{color:cl}}>{vl}</div><div className="plbl">{lb}</div></div>)}</div>
    ...
    <div className="dash-tabs">
      ...
      <div className={`dash-tab ${view==="overall"?"on":""}`} onClick={()=>setView("overall")}>

 Overall</div>

      {warehouses.map(wh=><div key={wh.id} className={`dash-tab ${view===wh.id?"on":""}`} onClick={()=>setView(wh.id)}>

 {wh.name}</div>)}

      <div className={`dash-tab ${view==="brand"?"on":""}`} onClick={()=>setView("brand")}>
 ...
 Brand-wise</div>
    ...
    {view==="overall"&&<div className="card"><div className="chd"><div><div className="ct">

 Tonnage-wise Bifurcation</div><div className="cs">All warehouses combined</div></div></div><TonBreakdown unitSet={units}/></div>}
      ...
      return <div key={wh.id}>
        ...
        <div className="wh-card">
          ...
          <div className="wh-card-hd">
            ...
            <div><div style={{fontSize:14,fontWeight:800}}>

 {wh.name}</div><div style={{fontSize:10.5,color:"var(--mu2)",marginTop:2}}>{wh.location}</div></div>
...
","Pending QC",ws.pend,"var(--am)"],["

","Available",ws.avail,"var(--gr)"],["

","Repair",ws.rep,"var(--rd)"],["

","Sold",ws.sold,"var(--gy)"]].map(([ic,lb,vl,cl])=><div key={lb} className="wh-stat"><div className="wh-stat-v" style={{color:cl}}>{vl}</div><div className="wh-stat-l">{ic} {lb}</div></div>)}

          </div>

        </div>

        <div className="card"><div className="chd"><div><div className="ct">

 Tonnage-wise — {wh.name}</div></div></div><TonBreakdown unitSet={wu}/></div>
...
function StockIntake({ units, lots, brands, tonnages, warehouses, onAdd, onTransfer, user }) {
 ...
 Stock Intake</div><div className="ps">Register incoming AC units · assign to warehouse</div></div>
...
      <div className="sc am"><div className="sl">Pending QC</div><div className="sv am">{units.filter(u=>u.status==="pending_qc").length}</div></div>
 ...
 Lot Overview</div></div></div>

      {Object.keys(lotGroups).length===0?<div className="empty"><div className="ei">

</div><div className="et">Create lots in Master first</div></div>:(
    ...
    <div className="card"><div className="chd"><div><div className="ct">All Units</div></div></div>
      ...
      <div className="tw"><table>
        ...
        <tbody>{units.map(u=><tr key={u.id}>
          ...
          <td>{u.rfidIn?<><span className="rtag">{u.rfidIn}</span><br/><span className="rtag">{u.rfidOut}</span></>:<span style={{fontSize:9.5,color:"var(--am)"}}>
 ...
 None</span>}</td>
          ...
          {user.role==="admin"&&<td>{u.status!=="sold"&&<button className="btn bpu bsm" onClick={()=>{setShowTx(u);setTxWh(u.warehouse);}}>
 ...
 Register AC Unit</div><div className="msu">All dropdowns from Master only</div>
...
  const progress = activeUnits.length>0 ? Math.round((verified.length/activeUnits.length)*100) : 0;
  ...
  if(submitted) return <div>
    ...
    <div className="ph"><div><div className="pt">

 Stock Verify</div></div></div>

    <div className="card" style={{textAlign:"center",padding:"36px"}}>

      <div style={{fontSize:40,marginBottom:10}}>{missing.length+unknownScans.length===0?"
...
"}</div>
 ...
 Stock Verify</div><div className="ps">Scan RFID tags manually — both indoor + outdoor must be scanned to verify a unit</div></div></div>
...
        return <div key={wh.id} className={`chip ${whFilter===wh.id?"on":""}`} onClick={()=>{setWhFilter(wh.id);clearAll();}}>
 ...
 {wh.name} ({cnt})</div>;
    ...
    <div className="sg sg4">
      ...
      <div className="sc gr"><div className="sl">

 Verified</div><div className="sv gr">{verified.length}</div></div>

      <div className="sc am"><div className="sl">

 Partial</div><div className="sv am">{partial.length}</div></div>

      <div className="sc rd"><div className="sl">

 Missing</div><div className="sv rd">{missing.length}</div></div>
    ...
    <div className="scan-input-wrap">
      ...
      <div style={{display:"flex",gap:8,justifyContent:"center",alignItems:"center",flexWrap:"wrap"}}>
        ...
        {scannedRfids.size>0&&<button className="btn brd bsm" onClick={clearAll}>
 ...
 Clear All</button>}
    ...
    <div className="card">
      ...
      {activeUnits.length===0
        ...
        ?<div className="empty"><div className="ei">

</div><div className="et">No active units in this warehouse</div></div>
...
    {unknownScans.length>0&&<div className="card">
      ...
      <div className="chd"><div className="ct" style={{color:"var(--am)"}}>

 Unknown Tags ({unknownScans.length})</div></div>
    ...
    {scannedRfids.size>0&&<div className="card">
      ...
      <div className="chd">
        ...
        <div><div className="ct">

 Submit Verification</div><div className="cs">{verified.length}/{activeUnits.length} verified · {missing.length} missing · {unknownScans.length} unknown</div></div>
      ...
      {user.role!=="admin"&&(missing.length>0||unknownScans.length>0)&&<div className="al al-w">

 Discrepancies found — only Admin can submit.</div>}

      <div style={{display:"flex",gap:7}}>

        {(user.role==="admin"||(missing.length===0&&unknownScans.length===0))&&<button className="btn bp" onClick={submitVerification}>

 Submit Verification</button>}

        <button className="btn bgh" onClick={reset}>
...
function QCModule({ units, warehouses, onUpdate, user }) {
 ...
 QC Module</div><div className="ps">Test each unit — pass to stock, fail to repair</div></div></div>
...
    </div>
    ...
    <div className="card"><div className="chd"><div className="ct">{SC[tab].icon} {SC[tab].label}</div></div>
      ...
      {rows.length===0?<div className="empty"><div className="ei">

</div><div className="et">No units here</div></div>:(
 ...
 None</span>}</td>
            ...
            {tab==="pending_qc"&&<td><div style={{display:"flex",gap:5}}><button className="btn bg2 bsm" onClick={()=>doPass(u)}>

 Pass</button><button className="btn br bsm" onClick={()=>setSel(u)}>

 Fail</button></div></td>}

            {tab==="under_repair"&&<td><button className="btn ba bsm" onClick={()=>onUpdate(u.id,{status:"pending_qc"})}>
 ...
 QC Failed</div><div className="msu"><span className="uid">{sel.id}</span> → Repair</div>
...
    const n=invCtr+1; const invNo=`INV-${String(n).padStart(4,"0")}`;
    ...
    showToast(`Sale recorded · Invoice ${invNo} 
...
`); setSellModal(null);
  ...
  const doDispatch=()=>{ const c=customers.find(c=>c.unitIds?.includes(dispModal.id)); onAddDispatch({id:genId(),unitId:dispModal.id,customerId:c?.id||"",invoiceNo:dispModal.invoiceNo,stage:"booked",...df}); showToast("Dispatch created 

"); setDispModal(null); };

  const markPay=u=>{ onUpdate(u.id,{paymentReceived:true}); showToast(`Payment received for ${u.id} 
...
`); setPayModal(null); };
 ...
 Sales</div><div className="ps">Manage sales, invoices and dispatch</div></div></div>
    ...
    <div className="filt">
      ...
      <div className={`chip ${tab==="available"?"on":""}`} onClick={()=>setTab("available")}>

 Available ({avail.length})</div>

      <div className={`chip ${tab==="sold"?"on":""}`} onClick={()=>setTab("sold")}>

 Sold ({sold.length})</div>
    ...
    <div className="card"><div className="chd"><div className="ct">{tab==="available"?"

 Available":"

 Sold"} ({rows.length})</div></div>

      {rows.length===0?<div className="empty"><div className="ei">

</div><div className="et">No units found</div></div>:(
        ...
        <div className="tw"><table>
          ...
          <tbody>{rows.map(u=>{ const d=dispatches.find(x=>x.unitId===u.id); return <tr key={u.id}>
            ...
            {tab==="sold"?<>
              ...
              <td>{u.paymentReceived?<span style={{color:"var(--gr)",fontSize:10.5}}>

 Received</span>:<span style={{color:"var(--am)",fontSize:10.5}}>

 Pending</span>}</td>

              <td><div style={{display:"flex",gap:4,flexWrap:"wrap"}}>

                <button className="btn bb bsm" onClick={()=>openInvoice(u)}>

</button>

                {!d&&<button className="btn bpu bsm" onClick={()=>setDispModal(u)}>

</button>}

                {d&&<StageBadge stage={d.stage}/>}

                {!u.paymentReceived&&<button className="btn bg2 bsm" onClick={()=>setPayModal(u)}>
...
              </div></td>
 ...
 Record Sale</div><div className="msu"><span className="uid">{sellModal.id}</span> · {sellModal.brand} {sellModal.tonnage} · <span className="price">{fmt(sellModal.salePrice)}</span></div>
 ...
 Create Dispatch</div><div className="msu"><span className="uid">{dispModal.id}</span></div>
 ...
 Confirm Payment</div><div className="msu"><span className="uid">{payModal.id}</span> · <span className="invno">{payModal.invoiceNo}</span> · <span className="price">{fmt(payModal.salePrice)}</span></div>

      <div className="mac"><button className="btn bgh" onClick={()=>setPayModal(null)}>Cancel</button><button className="btn bg2" onClick={()=>markPay(payModal)}>
 ...
 Confirm Payment Received</button></div>
    ...
    {invModal&&<div className="ov" onClick={e=>e.target===e.currentTarget&&setInvModal(null)}><div className="mo inv">
      ...
      <div className="inv-sheet">
        ...
        <div className="inv-hd"><div><div className="inv-co">

 CoolStock AC Supplies</div><div className="inv-cotag">Premium AC Sales · Your City</div><div style={{fontSize:10,color:"#6B7280",marginTop:3}}>GST: YOUR-GST-NO</div></div><div><div className="inv-no">{invModal.unit?.invoiceNo}</div><div className="inv-dt">{invModal.unit?.soldDate||today()}</div></div></div>

        <div className="inv-parties">

          <div><div className="inv-plbl">Bill To</div><div className="inv-pname">{invModal.customer?.name||"—"}</div>

            <div className="inv-pdet">{invModal.customer?.phone&&<div>

 {invModal.customer.phone}</div>}{invModal.customer?.email&&<div>

 {invModal.customer.email}</div>}{invModal.customer?.gst&&<div>GST: {invModal.customer.gst}</div>}{invModal.customer?.address&&<div>{invModal.customer.address}</div>}{invModal.customer?.city&&<div>{invModal.customer.city} {invModal.customer.pincode||""}</div>}</div>
...
        </div>
      ...
      <div className="mac">
        ...
        <button className="btn bb" onClick={()=>window.print()}>

 Print</button>

        <button className="btn bwa" onClick={()=>{ const p=(invModal.customer?.phone||"").replace(/\D/g,""); const msg=encodeURIComponent(`

 Invoice: ${invModal.unit?.invoiceNo}\n

 CoolStock AC Supplies\n\nDear ${invModal.customer?.name||"Customer"},\nProduct: ${invModal.unit?.brand} ${invModal.unit?.tonnage}\nUnit ID: ${invModal.unit?.id}\nAmount: ${fmt(invModal.unit?.salePrice)}\nThank you! 
...
function Dispatch({ dispatches, units, customers, warehouses, onUpdateDispatch, showToast }) {
  ...
  const adv=(d,ns)=>{ const u={stage:ns}; if(ns==="delivered")u.deliveredDate=today(); if(ns==="paid")u.paymentReceivedDate=today(); onUpdateDispatch(d.id,u); showToast(`${d.unitId} → ${DISPATCH_STAGES.find(s=>s.id===ns)?.label} 
...
`); setSm(null); };
 ...
 Dispatch</div><div className="ps">Track deliveries end-to-end</div></div></div>
    ...
    {filtered.length===0?<div className="empty"><div className="ei">

</div><div className="et">No dispatches found</div></div>:filtered.map(d=>{
      ...
      return <div key={d.id} className="dtcard">
        ...
        <div style={{fontSize:11.5,color:"var(--mu2)",marginBottom:10}}>{d.unit?.brand} {d.unit?.tonnage} · {d.customer?.name||"—"} · 

 {d.customer?.phone||"—"}</div>
...
            <div key={s.id} style={{display:"flex",flexDirection:"column",alignItems:"center",minWidth:56}}>
        ...
        <div style={{marginTop:9,display:"flex",gap:12,fontSize:10.5,color:"var(--mu2)",flexWrap:"wrap"}}>
          ...
          {d.deliveryPartner&&<span>

 {d.deliveryPartner}</span>}{d.trackingNo&&<span>

 {d.trackingNo}</span>}

          {d.bookedDate&&<span>

 {d.bookedDate}</span>}{d.deliveredDate&&<span>

 {d.deliveredDate}</span>}{d.paymentReceivedDate&&<span>

 {d.paymentReceivedDate}</span>}
 ...
 Dispatch Details</div><div className="msu"><span className="uid">{det.unitId}</span> · <span className="invno">{det.invoiceNo}</span></div>
...
function Customers({ customers, units, dispatches }) {
 ...
 Customers</div><div className="ps">All customer records from sales</div></div></div>
...
</div><div className="et">No customers yet</div></div>:(
...
            <td style={{fontSize:10.5}}>{c.createdDate}</td>
            ...
            <td><div style={{display:"flex",gap:5}}><button className="btn bb bsm" onClick={()=>setDet(c)}>View</button><button className="btn bwa bsm" onClick={()=>{ const p=(c.phone||"").replace(/\D/g,""); window.open(`https://wa.me/91${p}`,"_blank"); }}>
 ...
 {det.name}</div><div className="msu">Since {det.createdDate}</div>
...
"); };
 ...
 Warehouses"},{id:"lots",l:"

 Lots"},{id:"brands",l:"

 Brands"},{id:"tonnages",l:"
 ...
 Perms"}];
  ...
  const MasterSection = ({ type }) => {
    ...
    return <div className="card"><div className="chd"><div><div className="ct">{tab==="wh"?"

 Warehouse Master":tab==="lots"?"

 Lot Master":tab==="brands"?"

 Brand Master":"

 Tonnage Master"}</div><div className="cs">Only items here appear in Stock Intake dropdowns</div></div><button className="btn bp" onClick={()=>openAdd(type)}>+ Add</button></div>
 ...
 Master & Admin</div><div className="ps">Admin only</div></div></div>
...
    {(tab==="wh"||tab==="lots"||tab==="brands"||tab==="tonnages")&&<MasterSection type={tab}/>}
    ...
    {tab==="users"&&<div className="card"><div className="chd"><div><div className="ct">

 Users ({users.length})</div></div><button className="btn bp" onClick={()=>{setUmod({item:null});setUform({name:"",username:"",password:"",role:"technician",modules:["dashboard"],createdDate:today()});}}>+ Add</button></div>
    ...
    {tab==="perms"&&<div className="card"><div className="chd"><div className="ct">

 Permission Matrix</div></div>
      ...
      <div className="tw"><table><thead><tr><th>User</th>{ALL_MODULES.map(m=><th key={m.id} style={{textAlign:"center"}}>{m.icon}<br/><span style={{fontSize:8}}>{m.label.split(" ")[0]}</span></th>)}</tr></thead>
        ...
        <tbody>{users.map(u=><tr key={u.id}><td><b style={{fontSize:11.5}}>{u.name}</b><br/><span style={{fontSize:9.5,color:"var(--mu)"}}>{u.role}</span></td>{ALL_MODULES.map(m=><td key={m.id} style={{textAlign:"center"}}>{(u.modules||[]).includes(m.id)?<span style={{color:"var(--gr)"}}>

</span>:<span style={{color:"#1E293B"}}>✗</span>}</td>)}</tr>)}</tbody>
...
`); };
  ...
  const updateUnit=(id,ch)=>{ setUnits(p=>p.map(u=>u.id===id?{...u,...ch}:u)); const m={available:`${id} passed QC 

`,under_repair:`${id} → Repair 
...
`); };
  ...
  const onVerif=r=>{ setVerifs(p=>[r,...p]); showToast(r.discrepancies>0?`${r.discrepancies} discrepancy 

`:"Verification clear 

",r.discrepancies>0?"warn":"success"); };
    ...
    <div className="shell">
      ...
      <aside className={`sb ${sbOpen?"":"col"}`}>
        ...
        <div className="sb-top">
          ...
          <div className="sb-ico" onClick={toggleSb} title={sbOpen?"Collapse":"Expand"}>
